<?php
require_once str_replace("\\", "/", dirname(__FILE__)) .  '/dibs_fw_sb_params.php';

class dibs_fw_settingsBuilder extends dibs_fw_settingsBuilder_params {
    ...
    private function field($sKey, $sType, $sValue) {
        ...
        elseif($sType == 'checkbox') {
            $sConfig = '<select name="' . $sName . '">
                           ' . $this->arrayToOpts($this->cms_get_yesno(), $sValue) . '
                        </select>';
        }
    ...
    private function arrayToOpts($aOptArray, $sSelected = "") {
        ...
        foreach($aOptArray as $sKey => $sVal) {
            $sOpts .= '<option value="' . $sKey . '"' . 
                      (($sSelected == $sKey) ? " selected" : "") . '>' . $sVal . '</option>';            
        }
